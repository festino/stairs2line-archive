...
- Для Tumblr-файлов с суффиксом `_rN_` дата версии выводится как приблизительная верхняя граница: берётся `firstRebloggedAt` самого раннего сохранённого snapshot поста, в котором присутствует именно этот файл. Явные `createdAt` и `knownNotAfter` имеют более высокий приоритет.
...
Светлая тема является темой по умолчанию. При включённом JavaScript переключатель в шапке позволяет выбрать тёмную тему; выбор сохраняется в `localStorage` под ключом `archive-theme` и действует на всех страницах без изменения URL. Без JavaScript сайт остаётся в светлой теме, а переключатель скрыт.
