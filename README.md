...
- Галерея вынесена в отдельный раздел без карточек и подписей. Scope-фильтры представлены отдельными URL:
  ...
  - `/{lang}/gallery/all/` — все версии, включая `decorative`.
- `/{lang}/artworks/` теперь является списком artwork revisions: показываются только работы с несколькими версиями, а превью сравнивает раннюю и позднюю по дате версии в общей выровненной области. Scope-фильтра в этом разделе нет.
