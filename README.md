...
- `/{lang}/artworks/` является списком artwork revisions без scope-фильтра: `decorative` игнорируются, работы с несколькими недекоративными версиями сравниваются по ранней/поздней дате в общей выровненной области, а работы с единственной недекоративной картинкой собраны в конце списка.
...
- `MediaViewer` работает через универсальный provider/footer adapter по `mediaId`, `contextType`, `contextId`: ссылки на публикации ведут на внутренние страницы архива, оригинал остаётся отдельной внешней ссылкой; видео запускается только вручную, есть явная кнопка закрытия и ограниченные зоны листания.
...
- не запускает видео автоматически и показывает play-overlay;
- применяет artwork alignment/rotation/flip только там, где страница явно передала `data-viewer-align` (детальные страницы версий), но не в галерее/постах;
